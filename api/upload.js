...
const { resolveTableFieldName } = require('../server/utils/employeeNormalizer');
...
module.exports = async function uploadHandler(req, res) {
  ...
  requireAdminAuthIfConfigured(req, res, async () => {
    try {
      if (!req.headers['content-type'] || !String(req.headers['content-type']).includes('multipart/form-data')) {
        return sendJson(res, 415, { error: 'Expected multipart/form-data upload.' });
      }
      ...
      if (!sheetName) {
        return sendJson(res, 400, { error: 'The uploaded workbook is empty or unreadable.' });
      }

      const sheet = workbook.Sheets[sheetName];
      const rows = xlsx.utils.sheet_to_json(sheet, { defval: '' });
      if (!rows.length) {
        return sendJson(res, 400, { error: 'No rows were found in the uploaded file.' });
      }
      ...
      if (!employees.length) {
        return sendJson(res, 400, { error: 'The uploaded file did not produce any valid payroll rows.' });
      }
      ...
      if (error) {
        ...
        if (/row-level security|violates row-level security/i.test(msg)) {
          ...
          return sendJson(res, 502, { error: `Supabase schema needs updating. ${guidance}`, table: tableName, details: combinedDetails });
        }
